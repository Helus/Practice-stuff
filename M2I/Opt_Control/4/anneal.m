function [minimum,fval] = anneal(loss, parent, lb,ub, options)


% Eva Balsa Canto: Modification of code by joachim.vandekerckhove@psy.kuleuven.be
% to introduce upper and lower bounds on the decision variables
...
def = struct(...
        'CoolSched',@(T) (.8*T),...
        'Generator',@(x) (x+(randperm(length(x))==length(x))*randn/10),... 
        'InitTemp',1,...
        'MaxConsRej',1000,...
        'MaxSuccess',20,...
        'MaxTries',300,...
...
while ~finished;
    ...
    if itry >= max_try || success >= max_success;
        if T < minT || consec >= max_consec_rejections;
            ...
            T = cool(T,total);  % decrease T according to cooling schedule
            if report==2, % output
