...
% cuando ya sabemos los pesos óptimos
tic
fp =@(x) (x(1)-6)^2+(x(2)-7)^2+...
    mu_range(i_opt)*max([0 (-3*x(1)-2*x(2)+6)^2]) +...
    mu_range(j_opt)*max([0 (-x(1)+x(2)-3)^2]) +...
    mu_range(k_opt)*max([0 (x(1)+x(2)-7)^2]) +...
    mu_range(l_opt)*max([0 (2/3*x(1)-x(2)-4/3)^2]);
