...
norma= sqrt(trapz(t, error.^2))     % normal2(error,t);
%revisar grabacion min 20 24/2
...
% por si los puntos no son equidistantes  %revisar grabacion min 20 24/2
% derx_aprox=(x_aprox([3:num_pas+1])-x_aprox([1:num_pas-1]))./(t(3:num_pas+1)-t(1:num_pas-1));
errorder=derx_exacn(2:num_pas)-derx_aprox;
normader=sqrt(trapz(t(2:num_pas), errorder.^2));      % normal2(errorder,t(2:num_pas))
